...
## 2026-06-10 PC移植性向上（絶対パス依存の排除とフォルダ自動生成）

### 変更内容
1. **点群データフォルダパスの自動相対化およびEドライブ依存の排除 (`PointCloudLoader.cs`, `VRTestScene.unity`)**
   - 従来 `E:\VR\PointCloudData` に固定されていた点群データの外部フォルダパスをポータブルに調整。
   - `PointCloudLoader.cs` の初期値を空（`""`）に変更し、起動時（`Awake`）にパスが空、または指定された絶対パスのドライブ（例：Eドライブ）が存在しない場合、自動的に `Application.dataPath` を基準とした相対パス（プロジェクトの1つ上の階層の `PointCloudData` フォルダ）にフォールバック・解決するロジックを実装。
   - `VRTestScene.unity` 内のシリアライズされた絶対パス情報をクリア。これにより、他のPCにクローンしてUnityで開くだけで、そのPCの構成に合わせて正しいフォルダを指すよう自動調整されます。
2. **フォルダ自動生成機能の統合確認**
...
## 2026-06-10 UI表示の重なり解消と完全統合

### 変更内容
1. **右パネルの完全統合と重複描画の排除 (`PointCloudEditorUI.cs`, `PointCloudManager.cs`)**
   - `PointCloudEditorUI.cs` に残っていた古い「🧰 拡張機能パネル」の描画ロジックを完全に削除。
   - スケール校正とダウンサンプリングの各種操作ボタン、およびツールパネル表示トグル（アノテーションUI/モヤ処理UI）を、`PointCloudManager.cs` が描画する「☁ CloudCompare Unity機能パネル」へ完全に移植・統合。
   - これにより、右側に2つのパネルが重なって表示されてしまうUIの干渉問題（機能パネルの裏に拡張機能パネルが隠れる、重なる問題）を根本から解消。
2. **コンパイルエラーの完全解消とビルド確認**
   - 構文エラーをすべて修正し、UnityコマンドラインビルドにてC#コード全体のビルドが正常に通過（ExitCode 0）することを確認。

## 2026-06-10 アノテーションラベル属性に基づく部位別ダウンサンプリングとUI自動化

### 変更内容
1. **アノテーションラベル属性に基づく部位別ダウンサンプリング (`2_downsample.py`)**
   - 従来の「ファイル名・フォルダ名の文字列（leaf, stem等）」による判定を廃止。
   - `open3d.t.io.read_point_cloud` を使用し、PLYファイルからアノテーションラベル属性（`label`プロパティ）をロードする Tensor API 連携を実装。
   - Unity内のアノテーションデータ（茎=1、葉=2、果実=3 等）のラベル値ごとに点群を自動で分割し、それぞれ独立してダウンサンプリングを行う真の部位別ダウンサンプリングを実装。
2. **入出力フォルダの手動指定の廃止とワークフロー自動化 (`PointCloudEditorUI.cs`, `PointCloudEditor.cs`)**
   - UIのダイアログから「入力フォルダ」「出力フォルダ」の `TextField`（テキストフィールド）による手動入力を廃止。
   - 現在ロードされているPLYファイルのパス（`loader.GetFilePath()`）を自動的に検出して入力対象と決定。
   - 実行時、メモリ上の最新のアノテーション状態を自動で一時ファイル（`[filename]_labeled.ply`）としてバックグラウンドでエクスポートし、それを Python の入力として自動実行。出力ディレクトリも自動的に `[directory]/downsample/` に決定。
   - これにより、ユーザーはフォルダパスを意識することなく、ボクセルサイズと処理モードを選ぶだけでワンクリック実行できるようになりました。
