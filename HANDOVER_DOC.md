...
### 1. Python実行環境の自動セットアップ機能とバッチの追加 (2026-07-02)【最新】
- **Unity実行時の完全自動セットアップ機能**:
  - 他のPCでクローンした際に、手動でPython仮想環境（`.venv`）を構築しなくても、Unityからノイズ除去処理などを実行した瞬間に自動的に裏で構築を行ってから処理に移るように `PythonBridge.cs` を改修しました。
  - プログレスバーで進捗（「仮想環境を作成中...」「ライブラリをインストール中...」等）を通知するようにし、完了後はそのまま本来のプロセスを開始します。
- **ワンクリック用バッチファイル（setup_python_env.bat）**:
  - Unityを開く前に事前セットアップしたいユーザー向けに、ダブルクリックするだけで全自動で環境構築するバッチファイルをプロジェクトルートに追加しました。Pythonが未インストールの場合は公式サイトへの案内を出力します。

### 2. 3Dブラシ半径の最大値制限と操作感度の最適化 (2026-06-22)
- **ブラシ半径の最大値制限**:
  - 実用上の利便性を考慮し、アノテーション用3Dブラシの半径最大値を `2.0m` から `0.2m`（20cm）に制限しました。これにより、スライダーでの微調整がより細かく行えるようになりました。
- **操作のクランプと感度調整**:
  - `Alt + マウスホイール` によるブラシサイズ調整時に、サイズが `0.02m`〜`0.2m` の範囲外にならないようクランプ処理を追加しました。また、ホイールスクロールによるサイズ増減のステップ感度を `0.02m` 刻みに引き下げ、直感的に調整できるよう最適化しました。

### 2. 選択点のみのエクスポート機能および選択点数のリアルタイム表示 (2026-06-19)
- **選択点のみのエクスポート機能**:
  - アノテーションツール等で現在ハイライト（選択）されている点（`0x10000` フラグが立っている点）のみを抽出し、別ファイルとして保存する `ExportSelectedPoints()` メソッドを実装。出力ファイル名は `${fileNameWithoutExt}_selected.ply`。
  - アノテーションパネル最下部「PLYをエクスポート」の直下に「選択点のみエクスポート」ボタンを新設し、ASCII/Binaryの形式選択ダイアログと連携。
- **選択点数の表示**:
  - `PointCloudEditor.cs` の統計情報再計算処理 `RecalculateStats()` の全点ループ処理内で選択フラグをカウントするようにし、オーバーヘッドなしで選択中の点数をリアルタイム集計するプロパティ `SelectedPointCount` を実装。
  - アノテーションパネルの「選択オブジェクト操作」セクション内に「現在の選択点数: X 点」のラベルを常時表示（ツール選択が「なし（カメラ操作）」であっても常に確認可能）。選択数が1点以上の場合は緑色で強調表示するように改善。

### 3. パイプラインブロックの削除バグ修正およびUI表示状態の引き継ぎ保存 (2026-06-19)
- **パイプライン削除操作の不具合解消**:
...
### 4. 点群ピッキングの密度チェック機能（構造点優先・孤立点スキップ）の追加 (2026-06-17)
- **ノイズ孤立点の誤ピッキング問題の解消**:
